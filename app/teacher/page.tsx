    ...
    <main className="portalShell">
      ...
      <section className="heroGrid">
        <article className="nextCard">
          ...
          {nextLesson ? <><div className="nextMain"><div className="timeBadge"><Clock3 size={20} /><strong>{formatTime(nextLesson.start_time)}</strong><span>{formatTime(nextLesson.end_time)}</span></div><div><h3 style={{display:'flex',alignItems:'center',gap:6}}>{nextLesson.school}<a href={mapsUrl(nextLesson.school)} target="_blank" rel="noreferrer" style={{color:'inherit',opacity:.65,display:'flex'}} title="Open in Google Maps"><MapPin size={13}/></a></h3><p>{nextLesson.class_name} {lessonNumbers.has(nextLesson.id) ? `- ${ordinal(lessonNumbers.get(nextLesson.id)!)} lesson` : ''}</p><small><CalendarDays size={14} /> {linkedTeacherNames.length > 1 ? `${nextLesson.teacher_name ?? 'Unassigned'} · ` : ''}{nextLesson.lesson_date === todayKey ? 'Today' : new Intl.DateTimeFormat('en-SG', { weekday: 'long', day: 'numeric', month: 'short' }).format(parseLocalDate(nextLesson.lesson_date))}</small></div></div><div className="countdown"><i /><strong>{countdown}</strong></div></> : <div className="emptyNext"><CalendarDays size={34} /><strong>No upcoming lessons</strong><span>Your next assigned lesson will appear here.</span></div>}
        </article>
      ...
      <section className="upcomingCard"><div className="sectionHeading"><div><p>UPCOMING</p><h3>All scheduled lessons</h3></div><button onClick={scrollToSchedule}>Today <ArrowRight size={15}/></button></div><div className="upcomingGrid">{upcomingLessons.map((lesson)=><article key={lesson.id}><span>{new Intl.DateTimeFormat('en-SG',{weekday:'short',day:'numeric',month:'short'}).format(parseLocalDate(lesson.lesson_date))}</span><strong style={{display:'flex',alignItems:'center',gap:4}}>{lesson.school}<a href={mapsUrl(lesson.school)} target="_blank" rel="noreferrer" style={{color:'inherit',opacity:.65,display:'flex'}} title="Open in Google Maps"><MapPin size={11}/></a></strong><small>{lesson.class_name} {lessonNumbers.has(lesson.id) ? `- ${ordinal(lessonNumbers.get(lesson.id)!)} lesson` : ''}{linkedTeacherNames.length > 1 ? ` · ${lesson.teacher_name ?? 'Unassigned'}` : ''}</small><p><Clock3 size={13}/> {formatTime(lesson.start_time)}–{formatTime(lesson.end_time)}</p></article>)}{!upcomingLessons.length&&<div className="emptyUpcoming">No upcoming lessons assigned.</div>}</div></section>
