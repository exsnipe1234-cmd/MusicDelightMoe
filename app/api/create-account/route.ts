  ...
  try {
    ...
    if (!email || !password || !displayName) {
      return NextResponse.json(
        { error: 'Email address, password and display name are required.' },
        { status: 400 },
      );
    }
    ...
    if (role === 'teacher' && !teacherName) {
      return NextResponse.json(
        { error: 'A teacher account must be linked to a timetable.' },
        { status: 400 },
      );
    }
    if (role === 'admin' && teacherName) {
      return NextResponse.json(
        { error: 'Administrator accounts should not have a primary timetable.' },
        { status: 400 },
      );
    }
    ...
    if (profileError) {
      return NextResponse.json(
        {
          error: `Account created, but the timetable link could not be saved: ${profileError.message}`,
        },
        { status: 200 },
      );
    }
