...
export async function POST(request: Request) {
  try {
    ...
    if (!url || !publicKey || !process.env.OPENAI_API_KEY)
    ...
    if (!(image instanceof File) || !image.type.startsWith('image/'))
      return NextResponse.json(
        { error: 'Upload a PNG, JPEG, or WebP timetable image.' },
        { status: 400 },
      );
    if (image.size > 10 * 1024 * 1024)
      return NextResponse.json({ error: 'The image must be 10 MB or smaller.' }, { status: 400 });
      ...
      messages: [
        {
          role: 'system',
          content:
            'You extract dated school timetable lessons from images. Return JSON only. Never invent unreadable data.',
        },
        {
          role: 'user',
          content: [
    ...
    const lessons = Array.isArray(parsed.lessons)
      ? parsed.lessons.filter((lesson): lesson is DetectedLesson => {
          if (!lesson || typeof lesson !== 'object') return false;
          const row = lesson as DetectedLesson;
          return (
            validDate(row.date) &&
            validTime(row.startTime) &&
            validTime(row.endTime) &&
            typeof row.className === 'string' &&
            row.className.trim().length > 0 &&
            (typeof row.teacher === 'string' || row.teacher === null) &&
            (row.confidence === undefined ||
              (Number.isInteger(row.confidence) && row.confidence >= 0 && row.confidence <= 100)) &&
            (row.reviewReasons === undefined || Array.isArray(row.reviewReasons))
          );
        })
      : [];
    return NextResponse.json({
      monthName: typeof parsed.monthName === 'string' ? parsed.monthName : 'Image timetable',
      lessons,
    });
  } catch (error) {
    console.error('Image timetable OCR error:', error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Image timetable analysis failed.' },
      { status: 500 },
    );
  }
}
