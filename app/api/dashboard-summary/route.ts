...
export async function GET() {
  try {
    ...
    const [
      ...
      supabase
        .from('lessons')
        .select('id,lesson_date,teacher_name,school,class_name,start_time,end_time,unavailable')
        .eq('lesson_date', today)
        .eq('cancelled', false),
      supabase
        .from('lessons')
        .select('id,teacher_name,school,class_name,start_time,end_time,unavailable')
        .eq('lesson_date', tomorrow)
        .eq('cancelled', false),
      supabase
        .from('lessons')
        .select('id,lesson_date,teacher_name,school,class_name,start_time,end_time')
      ...
      supabase
        ...
        .select('id,teacher_name,start_date,end_date,status,reason')
        .eq('status', 'pending')
        .order('created_at'),
      supabase
        .from('replacement_tasks')
        .select(
          'id,original_teacher,lesson_date,school,class_name,start_time,end_time,status,replacement_teacher',
        )
      ...
      supabase
        ...
        .select('teacher_name,availability_type,start_date,end_date,reason')
        .eq('availability_type', 'leave')
    ...
    const optional = <T>(
      ...
      if (result.error)
        console.warn(`Dashboard summary could not read ${table}:`, result.error.message);
      return result.data ?? [];
    };
    ...
    for (let index = 0; index < upcomingRows.length; index++) {
      const first = upcomingRows[index];
      if (!first.teacher_name) continue;
      for (let other = index + 1; other < upcomingRows.length; other++) {
        const second = upcomingRows[other];
        if (
          first.teacher_name === second.teacher_name &&
    ...
    for (const request of requests) {
      if (request.start_date <= tomorrow && request.end_date >= tomorrow)
        unavailableTomorrow.add(request.teacher_name);
    }
    for (const leave of leaveRows) {
      if (
        leave.start_date &&
        leave.end_date &&
        leave.start_date <= tomorrow &&
        leave.end_date >= tomorrow
      )
        unavailableTomorrow.add(leave.teacher_name);
    }
    ...
    return NextResponse.json({
      ...
      alertCounts: {
        ...
        replacements: tasks.length,
      },
      suggestions: [
        ...(tasks.length
          ? [
              `${tasks.length} replacement task${tasks.length === 1 ? '' : 's'} still need coverage.`,
            ]
          : []),
        ...(unavailableTomorrow.size
          ? [`${[...unavailableTomorrow].join(', ')} unavailable tomorrow.`]
          : []),
        ...(tomorrowRows.some((row) => !row.teacher_name)
          ? ['There are unassigned lessons tomorrow.']
          : []),
      ].slice(0, 3),
    });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Could not build dashboard summary.' },
      { status: 500 },
    );
  }
}
