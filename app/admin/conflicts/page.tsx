...
import { AlertTriangle, ArrowLeft, CalendarDays, CalendarOff, CheckCircle2, Clock3, Loader2, MapPin, Sparkles, UserRoundX } from 'lucide-react';
...
type AvailabilityRow = { id:string; teacher_name:string; availability_type:'weekly'|'leave'; weekday:number|null; start_time:string|null; end_time:string|null; start_date:string|null; end_date:string|null; reason:string|null };
...
export default function ConflictCenterPage(){
  ...
  const load=async()=>{
    ...
    else{setLessons((lessonResult.data as LessonRow[])??[]);setMessage(`${lessonResult.data?.length??0} lessons checked against teacher availability.`);}
  ...
  useEffect(()=>{try{setReviewed(JSON.parse(localStorage.getItem(REVIEWED_KEY)||'[]'));}catch{setReviewed([]);}void load();},[]);

  const availabilityReason=(teacherName:string,lesson:LessonRow):string|null=>{
    const records=availability.filter(r=>r.teacher_name===teacherName);
    const leave=records.find(r=>r.availability_type==='leave'&&r.start_date&&r.end_date&&lesson.lesson_date>=r.start_date&&lesson.lesson_date<=r.end_date);
    ...
    const match=weekly.some(r=>r.weekday===weekdayFor(lesson.lesson_date)&&r.start_time&&r.end_time&&lesson.start_time.slice(0,5)>=r.start_time.slice(0,5)&&lesson.end_time.slice(0,5)<=r.end_time.slice(0,5));
    return match?null:'Lesson falls outside the teacher’s weekly availability.';
  };

  const conflicts=useMemo(()=>{
    const result:Conflict[]=[]; const groups=new Map<string,LessonRow[]>();
    for(const lesson of lessons){if(!lesson.teacher_name)continue;const key=`${lesson.teacher_name}|${lesson.lesson_date}`;groups.set(key,[...(groups.get(key)??[]),lesson]);}
    for(const [groupKey,group] of groups){const ordered=[...group].sort((a,b)=>a.start_time.localeCompare(b.start_time));for(let i=0;i<ordered.length-1;i+=1){const first=ordered[i],second=ordered[i+1];const gap=minutes(second.start_time)-minutes(first.end_time);const [teacher,date]=groupKey.split('|');if(gap<0)result.push({key:`${first.id}-${second.id}-overlap`,type:'overlap',teacher,date,first,second});else if(gap<30&&first.school.trim().toLowerCase()!==second.school.trim().toLowerCase())result.push({key:`${first.id}-${second.id}-tight`,type:'tight',teacher,date,first,second,gap});}}
    return result;
  },[lessons]);

  const availabilityIssues:AvailabilityIssue[]=lessons.filter(l=>l.teacher_name).map(l=>({lesson:l,reason:availabilityReason(l.teacher_name!,l)})).filter((x):x is AvailabilityIssue=>Boolean(x.reason));
  const activeConflicts=conflicts.filter(c=>!reviewed.includes(c.key));
  const overlaps=activeConflicts.filter(c=>c.type==='overlap');
  const tight=activeConflicts.filter(c=>c.type==='tight');
  const reviewedCount=conflicts.length-activeConflicts.length;
  const unassigned=lessons.filter(l=>!l.teacher_name);
  const manuallyUnavailable=lessons.filter(l=>l.unavailable);
  ...
  const rankedTeachers=(lesson:LessonRow):RankedTeacher[]=>{
    const workloads=new Map<string,number>(); teachers.forEach(t=>workloads.set(t.name,lessons.filter(l=>l.teacher_name===t.name).length));
    ...
    return teachers.flatMap(teacher=>{
      ...
      let score=70; const reasons=['Free at this lesson time','Within availability'];
      const sameSchoolDay=teacherLessons.some(other=>other.lesson_date===lesson.lesson_date&&other.school.trim().toLowerCase()===lesson.school.trim().toLowerCase());
      const adjacentSameSchool=teacherLessons.some(other=>other.lesson_date===lesson.lesson_date&&other.school.trim().toLowerCase()===lesson.school.trim().toLowerCase()&&(Math.abs(minutes(other.end_time)-minutes(lesson.start_time))<=90||Math.abs(minutes(lesson.end_time)-minutes(other.start_time))<=90));
      if(sameSchoolDay){score+=12;reasons.push('Already at the same school that day');}
      if(adjacentSameSchool){score+=8;reasons.push('Nearby lesson at the same school');}
      const load=workloads.get(teacher.name)??0; const balance=Math.round((1-load/maxLoad)*10); score+=balance;
      if(balance>=6)reasons.push('Lower current workload');
      const dayCount=teacherLessons.filter(other=>other.lesson_date===lesson.lesson_date).length;
      if(dayCount===0){score+=5;reasons.push('No other lessons that day');}
      return [{...teacher,score:Math.min(99,score),reasons}];
    }).sort((a,b)=>b.score-a.score||a.name.localeCompare(b.name));
  };

  const reassign=async(lesson:LessonRow,teacherName:string)=>{
    if(!teacherName)return;setMessage(`Assigning ${teacherName}...`);
  ...
  const markReviewed=(key:string)=>{const next=reviewed.includes(key)?reviewed:[...reviewed,key];setReviewed(next);localStorage.setItem(REVIEWED_KEY,JSON.stringify(next));setMessage('Conflict marked as reviewed and removed from active issues.');};

  return <main className="conflictShell">
    <header className="header"><div><Link href="/" className="back"><ArrowLeft size={17}/> Back to calendar</Link><p>ADMIN CONTROL PANEL</p><h1>Conflict Center</h1><span>Smart replacement suggestions rank teachers by availability, workload and school continuity.</span></div><button onClick={()=>void load()} disabled={loading}>{loading?<Loader2 className="spin" size={17}/>:<CheckCircle2 size={17}/>} Recheck</button></header>
    <section className="stats"><article className={issueCount?'danger':'good'}><span>Total issues</span><strong>{issueCount}</strong><small>{message}</small></article><article><span>Overlaps</span><strong>{overlaps.length}</strong><small>Same teacher, same time</small></article><article><span>Tight travel</span><strong>{tight.length}</strong><small>Under 30 min, different schools</small></article><article><span>Availability</span><strong>{availabilityIssues.length}</strong><small>Leave or outside weekly hours</small></article><article><span>Needs assignment</span><strong>{unassigned.length}</strong><small>{reviewedCount?`${reviewedCount} reviewed hidden`:'No teacher linked'}</small></article></section>
    ...
    {availabilityIssues.length>0&&<IssueSection title="Teacher availability conflicts" subtitle="Best-match teachers are ranked automatically." icon={<CalendarOff size={20}/>} items={availabilityIssues.map(({lesson,reason})=><LessonCard key={`availability-${lesson.id}`} lesson={lesson} choices={rankedTeachers(lesson)} onReassign={reassign} warning={reason}/>)}/>} 
