...
import { AlertTriangle, ArrowLeft, CalendarDays, CalendarOff, CheckCircle2, Clock3, Loader2, MapPin, UserRoundX } from 'lucide-react';
...
export default function ConflictCenterPage() {
  ...
  const load = async () => {
    ...
    if (!availabilityResult.error) setAvailability((availabilityResult.data as AvailabilityRow[]) ?? []);
  ...
  const availabilityReason = (teacherName: string, lesson: LessonRow): string | null => {
    const teacherRecords = availability.filter((record) => record.teacher_name === teacherName);
    const leave = teacherRecords.find((record) => record.availability_type === 'leave' && record.start_date && record.end_date && lesson.lesson_date >= record.start_date && lesson.lesson_date <= record.end_date);
    if (leave) return leave.reason ? `On leave: ${leave.reason}` : 'Teacher is on leave / unavailable on this date.';

    const weekly = teacherRecords.filter((record) => record.availability_type === 'weekly');
    if (weekly.length === 0) return null;
    const matchingWindow = weekly.some((record) => record.weekday === weekdayFor(lesson.lesson_date) && record.start_time && record.end_time && lesson.start_time.slice(0, 5) >= record.start_time.slice(0, 5) && lesson.end_time.slice(0, 5) <= record.end_time.slice(0, 5));
    return matchingWindow ? null : 'Lesson falls outside the teacher’s weekly availability.';
  };
  ...
  const availableTeachers = (lesson: LessonRow) => teachers.filter((teacher) => {
    const hasClash = lessons.some((other) => other.id !== lesson.id && other.teacher_name === teacher.name && clashes(lesson, other));
    return !hasClash && !availabilityReason(teacher.name, lesson);
  });
  ...
  return (
    <main className="conflictShell">
      <header className="header">
        <div><Link href="/" className="back"><ArrowLeft size={17}/> Back to calendar</Link><p>ADMIN CONTROL PANEL</p><h1>Conflict Center</h1><span>Overlaps, travel gaps and teacher availability are checked together.</span></div>
      ...
      {availabilityIssues.length > 0 && <IssueSection title="Teacher availability conflicts" subtitle="These lessons occur during leave or outside the teacher’s recurring availability." icon={<CalendarOff size={20}/>} items={availabilityIssues.map(({ lesson, reason }) => <LessonCard key={`availability-${lesson.id}`} lesson={lesson} choices={availableTeachers(lesson)} onReassign={reassign} warning={reason}/>)} />}
      ...
      {manuallyUnavailable.length > 0 && <IssueSection title="Teacher marked unavailable" subtitle="Assign a replacement teacher directly below." icon={<UserRoundX size={20}/>} items={manuallyUnavailable.map((lesson) => <LessonCard key={lesson.id} lesson={lesson} choices={availableTeachers(lesson)} onReassign={reassign}/>)} />}
      {unassigned.length > 0 && <IssueSection title="Unassigned lessons" subtitle="Only teachers who are free and available are shown." icon={<CalendarDays size={20}/>} items={unassigned.map((lesson) => <LessonCard key={lesson.id} lesson={lesson} choices={availableTeachers(lesson)} onReassign={reassign}/>)} />}

      <style jsx>{`
        .conflictShell{min-height:100vh;padding:34px;max-width:1500px;margin:auto;color:#eef2fb}.header{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:22px}.header p{margin:16px 0 7px;color:#8978ff;font-size:11px;font-weight:900;letter-spacing:.16em}.header h1{margin:0 0 7px;font-size:34px}.header span{color:#8995ad}.back{display:flex;align-items:center;gap:7px;color:#aa9cff}.header button{display:flex;align-items:center;gap:8px;padding:11px 15px;border-radius:12px;border:1px solid rgba(148,163,184,.14);background:#111a2d;color:#d4dced;font-weight:750;cursor:pointer}.stats{display:grid;grid-template-columns:repeat(5,1fr);gap:14px;margin-bottom:18px}.stats article,.allClear{border:1px solid rgba(148,163,184,.13);background:linear-gradient(145deg,rgba(20,27,48,.94),rgba(10,15,29,.92));border-radius:18px}.stats article{padding:18px;display:grid;gap:7px}.stats span,.stats small{color:#8491a9}.stats strong{font-size:28px}.stats .danger strong{color:#fb7185}.stats .good strong{color:#63d995}.allClear{text-align:center;padding:50px;color:#63d995}.allClear h2{color:#eef2fb;margin:12px 0 5px}.allClear p{color:#8491a9}.spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:1100px){.stats{grid-template-columns:repeat(2,1fr)}}@media(max-width:950px){.header{align-items:start}.conflictShell{padding:20px}}@media(max-width:560px){.stats{grid-template-columns:1fr}.header{display:grid}}
      `}</style>
    </main>
  );
}
...
function AssignmentControl({ lesson, choices, onReassign }: { lesson: LessonRow; choices: TeacherRow[]; onReassign: (lesson: LessonRow, teacherName: string) => void }) {
  ...
  return <div className="assignment"><select value={choice} onChange={(event) => setChoice(event.target.value)}><option value="">{choices.length ? 'Available teacher…' : 'No available teacher'}</option>{choices.map((teacher) => <option key={teacher.name} value={teacher.name}>{teacher.name}</option>)}</select><button disabled={!choice} onClick={() => onReassign(lesson, choice)}>Assign</button><style jsx>{`.assignment{display:flex;gap:8px;align-items:center}.assignment select{min-width:165px;padding:9px 10px;border-radius:9px;border:1px solid rgba(148,163,184,.16);background:#111a2d;color:#dce4f3}.assignment button{padding:9px 12px;border:0;border-radius:9px;background:#6653de;color:white;font-weight:750;cursor:pointer}.assignment button:disabled{opacity:.45;cursor:not-allowed}@media(max-width:850px){.assignment{align-items:stretch}.assignment select{flex:1}}`}</style></div>;
}
...
function LessonCard({ lesson, choices, onReassign, warning }: { lesson: LessonRow; choices: TeacherRow[]; onReassign: (lesson: LessonRow, teacherName: string) => void; warning?: string }) {
  return <article className="lessonCard"><Clock3 size={18}/><div><strong>{lesson.teacher_name ?? 'Unassigned'} · {niceDate(lesson.lesson_date)}</strong><span>{timeRange(lesson)} · {lesson.school}</span><small>{lesson.class_name}</small>{warning && <em>{warning}</em>}</div><AssignmentControl lesson={lesson} choices={choices} onReassign={onReassign}/><Link href="/admin/calendar">Open calendar</Link><style jsx>{`.lessonCard{display:flex;align-items:center;gap:12px;padding:14px;border-radius:13px;background:#0b1222;border:1px solid rgba(148,163,184,.1)}.lessonCard>div{display:grid;gap:3px;flex:1}.lessonCard span,.lessonCard small{color:#8995ad}.lessonCard em{margin-top:4px;color:#fb7185;font-size:11px;font-style:normal;font-weight:750}.lessonCard a{color:#9a8cff;font-weight:750;text-decoration:none}@media(max-width:850px){.lessonCard{align-items:stretch;flex-direction:column}}`}</style></article>;
}
