  ...
  return (
    <aside className="adminSidebar">
      ...
      <style jsx>{`
        ...
        .adminBrand{display:flex;align-items:center;gap:11px;padding:0 8px 25px}.adminBrandMark{width:42px;height:42px;border-radius:13px;display:grid;place-items:center;background:linear-gradient(135deg,#7857ff,#2ed4c7);box-shadow:0 12px 30px rgba(83,69,230,.3)}.adminBrand>div:last-child{display:grid;gap:2px}.adminBrand strong{font-size:15px}.adminBrand span{color:#78859d;font-size:11px}
        .adminNav{display:grid;gap:7px}.adminNav a{display:flex;align-items:center;gap:11px;padding:11px 12px;border-radius:11px;color:#8995ad;text-decoration:none;font-size:13px;font-weight:700;transition:.18s ease}.adminNav a:hover,.adminNav a.active{color:#fff;background:linear-gradient(90deg,rgba(120,87,255,.23),rgba(46,212,199,.07))}.adminNav a.active{box-shadow:inset 3px 0 #8b7cff}
        ...
        @media(max-width:900px){.adminSidebar{position:sticky;top:0;width:100%;height:auto;padding:11px 14px;display:grid;grid-template-columns:auto 1fr;align-items:center}.adminBrand{padding:0}.adminBrand>div:last-child,.adminSidebarFooter{display:none}.adminBrandMark{width:38px;height:38px}.adminNav{display:flex;justify-content:flex-end;overflow:auto}.adminNav a{padding:9px}.adminNav a span{display:none}}
      `}</style>
    </aside>
  );
}
