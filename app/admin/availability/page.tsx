...
export default function TeacherAvailabilityPage() {
  ...
  const load = async () => {
    ...
    if (error) {
      setMessage(
        error.message.includes('teacher_availability')
          ? 'Database setup required. Run supabase/teacher_availability.sql in the Supabase SQL Editor.'
          : error.message,
      );
      setRecords([]);
    } else {
  ...
  const weekly = selectedRecords
    .filter((item) => item.availability_type === 'weekly')
    .sort(
      (a, b) =>
        (a.weekday ?? 0) - (b.weekday ?? 0) ||
        (a.start_time ?? '').localeCompare(b.start_time ?? ''),
    );
  const leave = selectedRecords
    .filter((item) => item.availability_type === 'leave')
    .sort((a, b) => (a.start_date ?? '').localeCompare(b.start_date ?? ''));
  const selectedColour =
    teachers.find((teacher) => teacher.name === selectedTeacher)?.color ?? '#8b7cff';

  const addRecord = async () => {
    if (!selectedTeacher) return;
    if (mode === 'weekly' && startTime >= endTime) {
      setMessage('End time must be later than start time.');
      return;
    }
    if (mode === 'leave' && startDate > endDate) {
      setMessage('End date must be the same as or later than start date.');
      return;
    }
    setSaving(true);
    const payload: Omit<Availability, 'id'> =
      mode === 'weekly'
        ? {
  ...
  return (
    <main className="shell">
      <header className="header">
        ...
        <Link href="/admin/conflicts" className="conflicts">
      ...
      <section className="teacherStrip">
        {teachers.map((teacher) => (
          <button
            ...
            style={{ '--teacher': teacher.color } as React.CSSProperties}
          >
            <i />
            {teacher.name}
          </button>
      ...
      {loading ? (
        <div className="loading">
          <Loader2 className="spin" /> Loading availability...
        </div>
      ) : (
        <section className="grid">
          <div className="panel summary">
            <div className="teacherHeading">
              ...
              <div>
                <p>SELECTED TEACHER</p>
                <h2>{selectedTeacher || 'No teacher'}</h2>
              </div>
            </div>
            <div className="stats">
              <article>
              ...
              <article>
                ...
                <span>Leave blocks</span>
              </article>
            </div>
            <p className="help">
              Weekly windows represent when a teacher is normally available. Leave blocks override
              those windows for specific dates.
            </p>
          </div>
          ...
          <div className="panel formPanel">
            <div className="tabs">
              ...
              <button className={mode === 'leave' ? 'active' : ''} onClick={() => setMode('leave')}>
                <CalendarOff size={16} /> Leave / unavailable
              </button>
            </div>
            {mode === 'weekly' ? (
            ...
            <label>
              ...
              <input
                ...
                placeholder={
                  mode === 'weekly'
                    ? 'Optional, e.g. regular MOE availability'
                    : 'Optional, e.g. annual leave or reservist'
                }
              />
            </label>
            <button
            ...
            >
              {saving ? <Loader2 className="spin" size={17} /> : <Plus size={17} />} Add{' '}
              {mode === 'weekly' ? 'availability' : 'leave block'}
            </button>
          </div>
          ...
          <div className="panel records">
            <div className="sectionTitle">
              ...
              <div>
                ...
                <p>Normal recurring windows</p>
              </div>
            </div>
            {weekly.length === 0 ? (
              <div className="empty">No weekly availability added.</div>
            ) : (
              weekly.map((record) => (
                <Record
                  key={record.id}
                  record={record}
                  label={`${DAYS[record.weekday ?? 0]} · ${record.start_time?.slice(0, 5)}–${record.end_time?.slice(0, 5)}`}
          ...
          <div className="panel records">
            <div className="sectionTitle">
              ...
              <div>
                ...
                <p>Specific date overrides</p>
              </div>
            </div>
            {leave.length === 0 ? (
              <div className="empty">No leave blocks added.</div>
            ) : (
              leave.map((record) => (
                <Record
                  key={record.id}
                  record={record}
                  label={`${record.start_date}${record.end_date !== record.start_date ? ` → ${record.end_date}` : ''}`}
                  onDelete={removeRecord}
                />
              ))
            )}
...
function Record({
  ...
  return (
    <article className="record">
      <div>
        <strong>{label}</strong>
        <small>{record.reason || 'No note added'}</small>
      </div>
      <button onClick={() => onDelete(record)} aria-label="Delete">
        <Trash2 size={16} />
      </button>
      <style jsx>{`
        .record {
