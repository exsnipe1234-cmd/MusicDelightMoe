...
export default function CalendarPage() {
  ...
  const events = useMemo(() => visible.map((lesson) => { const teacherColour = lesson.cancelled ? '#f87171' : colour(lesson.teacher_name); return { id: lesson.id, title: lesson.school, start: `${lesson.lesson_date}T${lesson.start_time.slice(0, 5)}`, end: `${lesson.lesson_date}T${lesson.end_time.slice(0, 5)}`, backgroundColor: teacherColour, borderColor: teacherColour, textColor: '#1a1a2e', extendedProps: { ...lesson, teacherColour } }; }), [visible, colour]);
