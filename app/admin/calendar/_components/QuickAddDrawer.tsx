  ...
  return (
    <div className={styles.drawerBackdrop} onMouseDown={onClose}>
      <aside
        ...
        role="dialog"
        aria-label="Quick add multiple lessons"
      >
        <div className={styles.drawerHeader}>
          <div>
            ...
            <span>
              {quickRows.length} draft lesson{quickRows.length === 1 ? '' : 's'}
            </span>
          </div>
          <button onClick={onClose} aria-label="Close quick add">
            <X aria-hidden="true" />
          </button>
        </div>
        {copySourceCount > 0 && (
          <div className={styles.copyDatePanel}>
            ...
            <div>
              <input
                type="date"
                value={copyDateInput}
                onChange={(event) => onCopyDateInputChange(event.target.value)}
            ...
            <button
              ...
              disabled={!copyDates.length || saving}
            >
              Create copies
            </button>
          </div>
              ...
              <div className={styles.formGrid}>
                <label htmlFor={`${id}-date-${row.id}`}>
                  ...
                  <input
                    ...
                    type="date"
                    value={row.date}
                    onChange={(event) => onUpdateRow(row.id, { date: event.target.value })}
                  />
                </label>
                <div className={styles.timeRow}>
                  <label htmlFor={`${id}-start-${row.id}`}>
                    ...
                    <input
                      ...
                      type="time"
                      value={row.startTime}
                      onChange={(event) => onUpdateRow(row.id, { startTime: event.target.value })}
                    />
                  </label>
                  <label htmlFor={`${id}-end-${row.id}`}>
                    ...
                    <input
                      ...
                      type="time"
                      value={row.endTime}
                      onChange={(event) => onUpdateRow(row.id, { endTime: event.target.value })}
                    />
                  </label>
                </div>
                <label htmlFor={`${id}-school-${row.id}`}>
                  ...
                  <input
                    ...
                    value={row.school}
                    onChange={(event) => onUpdateRow(row.id, { school: event.target.value })}
                ...
                <label htmlFor={`${id}-class-${row.id}`}>
                  ...
                  <input
                    id={`${id}-class-${row.id}`}
                    list={`quick-class-options-${row.id}`}
                    value={row.className}
                    onChange={(event) => onUpdateRow(row.id, { className: event.target.value })}
                    placeholder="e.g. 4IN, Keyboard"
                  />
                ...
                <label htmlFor={`${id}-teacher-${row.id}`}>
                  Teacher
                  <select
                    id={`${id}-teacher-${row.id}`}
                    value={row.teacher}
                    onChange={(event) => onUpdateRow(row.id, { teacher: event.target.value })}
                  >
        ...
        <div className={styles.drawerActions}>
          <button className={styles.save} onClick={onSave} disabled={saving}>
            {saving
              ? 'Saving...'
              : `Save ${quickRows.length} lesson${quickRows.length === 1 ? '' : 's'}`}
          </button>
        </div>
      </aside>
    </div>
