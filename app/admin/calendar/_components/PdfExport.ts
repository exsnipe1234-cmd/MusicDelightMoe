...
export function openPrintPreview(title: string, body: string, landscape: boolean, exportRange: ExportRange, exportLessons: LessonRow[], onMessage: (msg: string) => void) {
  ...
  if (!popup) { URL.revokeObjectURL(url); onMessage('Please allow pop-ups to open the export preview.'); return; }
  setTimeout(() => URL.revokeObjectURL(url), 60000);
  onMessage(`${title} preview opened. Click \u201cPrint / Save PDF\u201d.`);
}

export function buildCalendarPdfBody(exportLessons: LessonRow[], exportRange: ExportRange, teacherColour: (name: string | null) => string): string {
...
export function buildSchedulePdfBody(exportLessons: LessonRow[], teacherColour: (name: string | null) => string): string {
  const groups = new Map<string, LessonRow[]>();
  exportLessons.forEach((lesson) => { const name = lesson.teacher_name ?? 'Unassigned'; groups.set(name, [...(groups.get(name) ?? []), lesson]); });
  const sections = [...groups.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([name, rows]) => {
    const c = teacherColour(name === 'Unassigned' ? null : name);
    const list = rows.sort((a, b) => a.lesson_date.localeCompare(b.lesson_date) || a.start_time.localeCompare(b.start_time)).map((lesson) => `<tr><td>${escapeHtml(pretty(lesson.lesson_date))}</td><td>${escapeHtml(lesson.start_time.slice(0,5))}\u2013${escapeHtml(lesson.end_time.slice(0,5))}</td><td>${escapeHtml(lesson.school)}</td><td>${escapeHtml(lesson.class_name)}</td></tr>`).join('');
