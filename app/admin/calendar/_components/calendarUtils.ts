...
export const blankQuickRow = (date: string, teacher = ''): QuickRow => ({
  id: Date.now(),
  date,
  startTime: '09:00',
  endTime: '10:00',
...
export const blankRecurring = (): RecurringDraft => ({
  school: '',
  className: '',
  startTime: '09:00',
  endTime: '10:00',
  ...
  endDate: key(new Date()),
  weekdays: [1],
});

export const key = (date: Date) =>
...
export const normalizeSchool = (value: string) =>
  value
    ...
    .replace(/\bprimary school\b\s+primary school\b/g, 'primary school')
    .replace(/\s+[a-z]?\d{1,2}[a-z]{0,3}\s*$/g, '')
    .replace(/\s+/g, ' ')
    .trim();
 ...
 */
export const defaultTeacherColours: Record<string, string> = {
  ashley: '#d6b94c',
  audrey: '#c77bd5',
  claris: '#59b879',
  edward: '#55b9e6',
