...
const lessonStart = (lesson: AffectedLesson) =>
  (lesson.start_time ?? lesson.startTime ?? '').slice(0, 5);
const lessonEnd = (lesson: AffectedLesson) => (lesson.end_time ?? lesson.endTime ?? '').slice(0, 5);
const lessonClass = (lesson: AffectedLesson) =>
  lesson.class_name ?? lesson.className ?? 'MOE programme';
...
export default function AdminRequestsPage() {
  ...
  const loadRequests = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'id,teacher_name,start_date,end_date,reason,remarks,affected_lessons,status,admin_note,replacement_summary,created_at',
      )
    ...
    if (error) {
      ...
      setMessage(
        `${rows.filter((request) => request.status === 'pending').length} pending request${rows.filter((request) => request.status === 'pending').length === 1 ? '' : 's'}.`,
      );
    }
    setLoading(false);
  };
  ...
  const affectedCount = requests
    .filter((request) => request.status === 'pending')
    .reduce((sum, request) => sum + (request.affected_lessons?.length ?? 0), 0);
  const teacherCount = new Set(
    requests
      .filter((request) => request.status === 'pending')
      .map((request) => request.teacher_name),
  ).size;
  ...
  return (
    <main className="requestShell">
      <header className="requestHeader">
        <div>
          <Link href="/admin/calendar" className="backLink">
        ...
        <button className="refreshButton" onClick={() => void loadRequests()} disabled={loading}>
      ...
      <section className="summaryGrid">
        ...
        <article>
          <UserRound size={21} />
          <span>Teachers affected</span>
          <strong>{teacherCount}</strong>
        </article>
      </section>

      <div
        className={`statusMessage ${message.toLowerCase().includes('could not') || message.toLowerCase().includes('required') || message.toLowerCase().includes('not available') ? 'error' : ''}`}
      >
      ...
      <section className="toolbar">
        {(
          ['pending', 'approved', 'replacement_assigned', 'rejected', 'cancelled', 'all'] as const
        ).map((value) => (
          <button
            key={value}
      ...
      <section className="requestList">
          ...
          <article className="requestCard" key={request.id}>
            <div className="requestTop">
              <div className="identity">
                <div className="avatar">{request.teacher_name.slice(0, 2).toUpperCase()}</div>
                <div>
                  <h2>{request.teacher_name}</h2>
                  <span>
                    Submitted{' '}
                    {new Intl.DateTimeFormat('en-SG', {
                      day: 'numeric',
            ...
            <div className="requestInfo">
              ...
              <div>
                <span>Affected</span>
                <strong>
                  {request.affected_lessons?.length ?? 0} lesson
                  {request.affected_lessons?.length === 1 ? '' : 's'}
                </strong>
              </div>
            </div>
              ...
              {(request.affected_lessons ?? []).map((lesson, index) => (
                <div className="lessonRow" key={lesson.id ?? `${request.id}-${index}`}>
                  <div className="dateBox">
                    ...
                    <strong>
                      {lessonDate(lesson) ? prettyDate(lessonDate(lesson)) : 'Date unavailable'}
                    </strong>
                  </div>
                  <div>
                    <strong>{lesson.school ?? 'School unavailable'}</strong>
                    <span>{lessonClass(lesson)}</span>
                  </div>
                  <div className="time">
                    <Clock3 size={14} />
                    {lessonStart(lesson) || '--:--'}
                    {lessonEnd(lesson) ? `–${lessonEnd(lesson)}` : ''}
                  </div>
                </div>
              ))}
              {!request.affected_lessons?.length && (
                <div className="emptyLessons">No lesson snapshot was saved with this request.</div>
              )}
            </div>
            ...
            <div className="actions">
              <Link
                href={`/admin/conflicts?teacher=${encodeURIComponent(request.teacher_name)}&from=${request.start_date}&to=${request.end_date}`}
              ...
              {request.status === 'pending' && (
                <>
                  <button
                    className="reject"
                    onClick={() => void updateStatus(request, 'rejected')}
              ...
              {request.status === 'approved' && (
                <button
                  className="assigned"
                  onClick={() => void updateStatus(request, 'replacement_assigned')}
                  disabled={savingId === request.id}
                >
                  <CheckCircle2 size={16} /> Mark replacement assigned
                </button>
              )}
            </div>
          </article>
        ...
        {!loading && !visibleRequests.length && (
          <div className="emptyState">
            ...
            <strong>No requests in this section</strong>
            <span>New teacher submissions will appear here automatically.</span>
          </div>
        )}
      </section>
      ...
      <style jsx>{`
        ...
        .avatar {
          ...
          display: grid;
          place-items: center;
          background: linear-gradient(135deg, #705ae5, #4e3bb8);
          font-weight: 850;
        }
        .identity h2 {
          margin: 0 0 4px;
        ...
        .statusPill.replacement_assigned {
        ...
        .remarks {
          padding: 13px;
          border-left: 3px solid #7665e8;
          background: rgba(118, 101, 232, 0.07);
          border-radius: 9px;
          margin-bottom: 17px;
        }
        .remarks p {
        ...
        .replacementButton {
          background: #17213a;
          color: #aa9eff;
          border: 1px solid rgba(129, 112, 242, 0.2);
        }
        .reject {
          background: rgba(251, 113, 133, 0.1);
          color: #fb7185;
        ...
        .assigned {
          background: linear-gradient(135deg, #0e9f79, #08775d);
        }
        .emptyState {
          padding: 48px;
          display: grid;
