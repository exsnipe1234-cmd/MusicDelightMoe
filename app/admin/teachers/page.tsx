...
export default function TeacherManagementPage() {
  ...
  const loadData = async () => {
    ...
    const [
      ...
      supabase
        .from('profiles')
        .select('id,email,display_name,role,teacher_name,active')
        .order('display_name'),
    ...
    if (profileError || teacherError || accessError) {
      setMessage(
        profileError?.message ||
          ...
          accessError?.message ||
          'Could not load teacher management. Run the multi-timetable Supabase migration first.',
      );
      setLoading(false);
      return;
    }
    ...
    setDrafts(
      Object.fromEntries(
        loadedProfiles.map((profile) => {
          const linked = loadedAccess
            .filter((row) => row.profile_id === profile.id)
            .map((row) => row.teacher_name);
          const teacherNames = Array.from(
            new Set([...(profile.teacher_name ? [profile.teacher_name] : []), ...linked]),
          );
          return [
            profile.id,
            {
  ...
  const saveProfile = async (profile: Profile) => {
    ...
    const linkedTeacherNames = Array.from(
      new Set([...(draft.teacher_name ? [draft.teacher_name] : []), ...draft.teacher_names]),
    );
    ...
    setDrafts((current) => ({
      ...current,
      [profile.id]: { ...draft, teacher_names: linkedTeacherNames },
    }));
    setProfiles((current) =>
      current.map((item) =>
        item.id === profile.id
  ...
  const addTeacher = async (event: FormEvent) => {
    ...
    const { data, error } = await supabase
      .from('teachers')
      .insert({ name, color: newTeacher.color })
      .select('name,color')
      .single();
    if (error) {
      setMessage(
        error.code === '23505'
          ? 'That teacher already exists.'
          : `Could not add teacher: ${error.message}`,
      );
      return;
    }
    setTeachers((current) =>
      [...current, data as Teacher].sort((a, b) => a.name.localeCompare(b.name)),
    );
  ...
  const createAccount = async (event: FormEvent) => {
    ...
    setCreating(true);
    setMessage(
      `Creating account for ${createDraft.display_name.trim() || createDraft.email.trim()}…`,
    );
    try {
      ...
      if (!response.ok) {
        setMessage(`Could not create account: ${body.error}`);
        return;
      }
      ...
      setMessage(
        `Could not create account: ${error instanceof Error ? error.message : 'Network error'}`,
      );
    } finally {
      setCreating(false);
    }
  ...
  return (
    <main className="manageShell">
      <header className="manageHeader">
        ...
        <button className="refreshButton" onClick={() => void loadData()} disabled={loading}>
      ...
      <section className="summaryGrid">
        ...
        <article>
          ...
          <strong>{profiles.filter((profile) => profile.role === 'admin').length}</strong>
        </article>
        <article>
          <UserCog size={20} />
          <span>Active teachers</span>
          <strong>
            {profiles.filter((profile) => profile.role === 'teacher' && profile.active).length}
          </strong>
        </article>
      </section>

      <div
        className={`status ${message.toLowerCase().includes('could not') || message.toLowerCase().includes('required') ? 'error' : ''}`}
      >
      ...
      <section className="panel">
        <div className="panelHeading">
          <div>
            <p className="eyebrow">LOGIN ACCOUNTS</p>
            <h2>Users and permissions</h2>
          </div>
          <small>
            Create new login users in Supabase Authentication; they will appear here automatically.
          </small>
        </div>
              ...
              <article className="accountCard" key={profile.id}>
                <div className="accountIdentity">
                  <div className="avatar">
                    {(draft.display_name || profile.email || '?').slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <strong>{draft.display_name || 'Unnamed account'}</strong>
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setDrafts({
                          ...drafts,
                          [profile.id]: { ...draft, display_name: event.target.value },
                        })
                      }
                    />
                  </label>
                  <label>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setDrafts({
                          ...drafts,
                          [profile.id]: { ...draft, role: event.target.value as Draft['role'] },
                        })
                      }
                    >
                  ...
                  <label className="activeLabel">
                    <input
                      ...
                      onChange={(event) =>
                        setDrafts({
                          ...drafts,
                          [profile.id]: { ...draft, active: event.target.checked },
                        })
                      }
                    />
                    <span>{draft.active ? 'Active' : 'Disabled'}</span>
                  </label>
                  <div className="timetableAccess">
                    <span>Visible timetables</span>
                    <div>
                      {teachers.map((teacher) => {
                        const isPrimary = teacher.name === draft.teacher_name;
                        const checked = draft.teacher_names.includes(teacher.name) || isPrimary;
                        return (
                          <label key={teacher.name} className={isPrimary ? 'primaryTeacher' : ''}>
                            <input
                              type="checkbox"
                              checked={checked}
                              disabled={isPrimary}
                              onChange={() => toggleTeacherAccess(profile.id, teacher.name)}
                            />
                ...
                <div className="actions">
                  <button
                    className="secondary"
                    onClick={() => void sendReset(profile)}
                    disabled={!profile.email || savingId === profile.id}
                  >
      ...
      <section className="panel addPanel">
        <div className="panelHeading">
          ...
          <small>
            This adds a timetable name. Create the login separately in Supabase Authentication, or
            use the form below.
          </small>
        </div>
        <form onSubmit={addTeacher} className="addForm">
          <label>
            Teacher name
            <input
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setNewTeacher({ ...newTeacher, color: event.target.value })}
            />
          </label>
      ...
      <section className="panel addPanel">
        <div className="panelHeading">
          <div>
            <p className="eyebrow">CREATE LOGIN</p>
            <h2>Create new account</h2>
          </div>
          <small>
            This creates a login directly in the dashboard. The account will appear above after
            creation.
          </small>
        </div>
        <form
        ...
        >
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setCreateDraft({ ...createDraft, role: event.target.value as CreateDraft['role'] })
              }
            >
              <option value="teacher">Teacher</option>
              <option value="admin">Admin</option>
          ...
          {createDraft.role === 'teacher' && (
            <label>
              Timetable name
              <select
                value={createDraft.teacher_name}
                onChange={(event) =>
                  setCreateDraft({ ...createDraft, teacher_name: event.target.value })
                }
              >
                <option value="">Not linked</option>
                {teachers.map((teacher) => (
          ...
          >
            {creating ? <Loader2 className="spin" size={16} /> : <Plus size={16} />} Create account
          </button>
        </form>
      </section>
      ...
      <style jsx>{`
        ...
        .primary {
          background: linear-gradient(135deg, #715be8, #5140c6);
          color: white;
        }
        .summaryGrid {
          display: grid;
        ...
        .status {
          ...
          background: linear-gradient(145deg, rgba(20, 27, 48, 0.94), rgba(10, 15, 29, 0.92));
          border-radius: 17px;
        }
        .summaryGrid article {
          padding: 18px;
        ...
        .accountIdentity > div:last-child {
          display: grid;
          gap: 4px;
        }
        ...
        .timetableAccess label.primaryTeacher {
          border-color: rgba(129, 112, 242, 0.35);
          background: rgba(129, 112, 242, 0.1);
        }
        .timetableAccess i {
          width: 8px;
          height: 8px;
