...
export default function TeacherManagementPage() {
  ...
  const loadData = async () => {
    ...
    const [{ data: profileRows, error: profileError }, { data: teacherRows, error: teacherError }] = await Promise.all([
    ...
    if (profileError || teacherError) {
      setMessage(profileError?.message || teacherError?.message || 'Could not load teacher management.');
      setLoading(false);
      return;
    }
    ...
    setDrafts(Object.fromEntries(loadedProfiles.map((profile) => [profile.id, {
  ...
  const saveProfile = async (profile: Profile) => {
    ...
    if (draft.role === 'teacher' && !draft.teacher_name) {
      setMessage('Teacher accounts must be linked to a teacher name.');
      return;
    }
            ...
            return (
              <article className="accountCard" key={profile.id}>
                ...
                <div className="fields">
                  ...
                  <label>Linked timetable<select value={draft.teacher_name} onChange={(event) => setDrafts({...drafts,[profile.id]:{...draft,teacher_name:event.target.value}})}><option value="">Not linked</option>{teachers.map((teacher) => <option key={teacher.name} value={teacher.name}>{teacher.name}</option>)}</select></label>
