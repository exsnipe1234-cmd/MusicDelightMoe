...
type ExistingRow = { lesson_date: string; school: string; class_name: string; start_time: string; end_time: string; teacher_name: string | null };
...
export default function ImportPage() {
  ...
  const saveImport = async () => {
    ...
    const { data: existing, error: readError } = await supabase.from('lessons').select('lesson_date,school,class_name,start_time,end_time,teacher_name').in('lesson_date', dates);
    if (readError) { setStatus('error'); setMessage(`Could not check existing lessons: ${readError.message}`); return; }
    const existingKeys = new Set(((existing ?? []) as ExistingRow[]).map((row) => lessonKey({ date: row.lesson_date, startTime: row.start_time, endTime: row.end_time, school: row.school, className: row.class_name, teacher: row.teacher_name })));
    const newLessons = lessons.filter((lesson) => !existingKeys.has(lessonKey(lesson)));
    ...
    setStatus('saved'); setMessage(`${newLessons.length} lessons saved to Supabase. ${lessons.length - newLessons.length} duplicates skipped.`); setTimeout(() => router.push('/'), 900);
  };
  return <main className="importShell"><header className="importHeader"><Link href="/" className="backLink"><ArrowLeft size={17}/> Back to calendar</Link><div><p>SUPABASE IMPORT</p><h1>Import MOE timetable</h1><span>Upload, review and save the detected lessons directly to the shared cloud calendar.</span></div></header><section className="importCard uploadCard"><label className="dropZone"><input type="file" accept="application/pdf,.pdf" onChange={handleFile}/><Upload size={34}/><strong>{fileName || 'Choose MOE calendar PDF'}</strong><span>PDF only · processed in your browser</span></label><div className={`statusBox ${status}`}>{status === 'reading' || status === 'saving' ? <Loader2 className="spin" size={20}/> : status === 'error' ? <XCircle size={20}/> : <CheckCircle2 size={20}/>}<span>{message || 'No PDF selected yet.'}</span></div></section>{lessons.length > 0 && <><section className="importStats"><article><span>Calendar</span><strong>{monthName}</strong></article><article><span>Lessons detected</span><strong>{lessons.length}</strong></article><article><span>Assigned</span><strong>{assignedCount}</strong></article><article><span>Needs review</span><strong>{lessons.length-assignedCount}</strong></article></section><section className="importCard previewCard"><div className="previewHeader"><div><p>IMPORT PREVIEW</p><h2>Detected lessons</h2></div><button onClick={saveImport} disabled={status === 'saving'}>{status === 'saving' ? 'Saving…' : `Import ${lessons.length} lessons`}</button></div><div className="tableWrap"><table><thead><tr><th>Date</th><th>Time</th><th>School</th><th>Class / programme</th><th>Teacher</th><th/></tr></thead><tbody>{lessons.map((lesson) => <tr key={lesson.id}><td>{lesson.date}</td><td>{lesson.startTime} - {lesson.endTime}</td><td>{lesson.school}</td><td>{lesson.className}</td><td>{lesson.teacher ?? <em>Unassigned</em>}</td><td><button className="removeButton" onClick={() => setLessons((current) => current.filter((item) => item.id !== lesson.id))}>Remove</button></td></tr>)}</tbody></table></div></section></>}<style jsx>{`.importShell{min-height:100vh;padding:38px;max-width:1500px;margin:auto}.importHeader{display:grid;gap:24px;margin-bottom:24px}.backLink{display:flex;align-items:center;gap:8px;color:#9aa7bf;text-decoration:none;width:max-content}.importHeader p,.previewHeader p{margin:0 0 6px;color:#8174ff;font-size:11px;font-weight:800;letter-spacing:.15em}.importHeader h1{margin:0 0 8px;font-size:34px}.importHeader span{color:#8995ad}.importCard,.importStats article{border:1px solid rgba(148,163,184,.14);background:linear-gradient(145deg,rgba(20,27,48,.92),rgba(11,16,31,.86));border-radius:18px}.uploadCard{padding:20px}.dropZone{min-height:180px;border:1px dashed rgba(129,116,255,.5);border-radius:15px;display:grid;place-items:center;align-content:center;gap:9px;color:#c8c2ff;cursor:pointer;background:rgba(120,87,255,.06)}.dropZone input{display:none}.dropZone span{font-size:12px;color:#76839b}.statusBox{margin-top:14px;padding:12px 14px;border-radius:11px;display:flex;align-items:center;gap:9px;background:#0b1222;color:#8995ad}.statusBox.error{color:#fb7185}.statusBox.ready,.statusBox.saved{color:#70d28c}.spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.importStats{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:16px 0}.importStats article{padding:17px;display:grid;gap:7px}.importStats span{color:#8793aa;font-size:12px}.importStats strong{font-size:21px}.previewCard{overflow:hidden}.previewHeader{padding:18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(148,163,184,.1)}.previewHeader h2{margin:0}.previewHeader button{border:0;border-radius:11px;padding:11px 16px;background:linear-gradient(135deg,#6c56e8,#5544cf);color:white;font-weight:700;cursor:pointer}.previewHeader button:disabled{opacity:.6;cursor:wait}.tableWrap{overflow:auto;max-height:580px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{padding:11px 13px;text-align:left;border-bottom:1px solid rgba(148,163,184,.08);white-space:nowrap}th{position:sticky;top:0;background:#0c1324;color:#7f8ca5;text-transform:uppercase;font-size:10px}td{color:#c1cadb}td em,.removeButton{color:#fb7185}.removeButton{border:0;background:transparent;cursor:pointer}@media(max-width:900px){.importShell{padding:20px}.importStats{grid-template-columns:repeat(2,1fr)}}`}</style></main>;
}
