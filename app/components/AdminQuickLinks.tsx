    ...
    <div className={open ? 'quickActions open' : 'quickActions'}>
      ...
      <div className="quickPanel" aria-hidden={!open}>
        <div className="quickPanelHeader">
          ...
          <button onClick={() => setOpen(false)} aria-label="Close quick actions">
            <X size={18} />
          </button>
        </div>
        <div className="quickLinks">
          {links.map(({ href, label, icon: Icon, tone }) => (
            <Link
              key={href}
              href={href}
              className={`quickLink ${tone} ${pathname === href ? 'active' : ''}`}
            >
              <Icon size={17} />
              <span>{label}</span>
            </Link>
      ...
      <style jsx global>{`
        ...
        .quickPanelHeader strong {
          font-size: 14px;
        }
        .quickPanelHeader button {
          width: 32px;
          height: 32px;
          display: grid;
          place-items: center;
        ...
        .quickLink.danger {
          background: linear-gradient(145deg, rgba(239, 68, 68, 0.28), rgba(185, 28, 28, 0.18));
        }
        .quickLink.warning {
          background: linear-gradient(145deg, rgba(245, 158, 11, 0.28), rgba(217, 119, 6, 0.18));
        }
        .quickLink.dangerSoft {
          background: linear-gradient(145deg, rgba(226, 88, 88, 0.28), rgba(183, 55, 55, 0.18));
        }
        .quickLink.info {
          background: linear-gradient(145deg, rgba(14, 165, 168, 0.28), rgba(79, 70, 229, 0.18));
        }
        .quickLink.purple {
          background: linear-gradient(145deg, rgba(124, 92, 255, 0.28), rgba(168, 85, 247, 0.18));
        }
